...
static
void InitEndpoints() {
	InitEPSize(PS4ARDUINO_TX_ENDPOINT, EP_TYPE_INTERRUPT_IN,  2, 64);  // Control Data Send
	InitEPSize(PS4ARDUINO_RX_ENDPOINT, EP_TYPE_INTERRUPT_OUT, 2, 64);  // Control Data Receive
	InitEPSize(5, EP_TYPE_INTERRUPT_IN,  2, 64);  // Expansion Interface NACK (avoid config reset)

	UERST = 0x7E;  // Reset endpoints
	UERST = 0;     // End reset

	SetEP(PS4ARDUINO_TX_ENDPOINT);
	UEIENX |= (1 << TXINE);

	SetEP(PS4ARDUINO_RX_ENDPOINT);
	UEIENX |= (1 << RXOUTE);
}
...
ISR(USB_COM_vect)
{
	...
	if (UEINTX & (1 << TXINI)) {  // If TX buffer is ready
		...
		if (PS4ArduinoUSB::SendCallback != nullptr) {
			PS4ArduinoUSB::SendCallback();  // Call callback function if it exists
		}
	}
	...
	if (UEINTX & (1 << RXOUTI)) {  // If data received...
		...
		if (PS4ArduinoUSB::RecvCallback != nullptr) {
			PS4ArduinoUSB::RecvCallback();  // Call callback function if it exists
		}
	}
    SetEP(0);
	if (!ReceivedSetupInt())
